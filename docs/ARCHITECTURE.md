...
- 改写：V4-Pro 只返回结构化补丁，V4-Flash 独立检查语义漂移；同一 Agent 会话可以基于上一版建议继续调整，但原始选段始终是不变的安全锚点。数字、比例、引文、引用标记、URL、缩写和专有项仍由服务端规则保护。
- 首次修改：每个未过期且包含风险片段的检测结果都显示“一键降低”，既往已接受补丁不得隐藏该入口。服务端从当前不可变版本及其最新成功检测中派生全部风险段落，浏览器不能注入替代正文或目标。真实 DeepSeek 模式使用学术化 Humanizer 规则一次处理这些段落，V4 Flash 逐段验证后原子保存为一个可恢复的新版本并使本轮检测过期；任一结构、保护内容或语义校验失败都会整体失败关闭。Mock 模式只生成一条可审阅预览，不自动改动正文。随后恢复逐条生成、审阅、接受或拒绝的现有模式；该动作不自动复检，也不承诺具体检测分数。
- 上下文：默认只发送选段所在段落；段落、小节和全文上下文均由服务端从会话绑定的不可变版本中派生，前端不能注入任意正文。全文上下文需要用户显式确认，并受字符上限约束。
- 决策：每个 Agent 会话同一时间只有一条待处理建议；成功生成下一版后，上一版标为 `superseded`。只有用户明确接受才创建新文稿版本；首次“一键降低”点击本身构成对该次校验通过补丁的明确接受。拒绝、继续调整和查看建议都不会改动正文或自动复检。
