...
- 改写：V4-Pro 只返回结构化补丁，V4-Flash 独立检查语义漂移；同一 Agent 会话可以基于上一版建议继续调整，但原始选段始终是不变的安全锚点。模型不直接覆盖正文，数字、比例、引文、引用标记、URL、缩写和专有项仍由服务端规则保护。
- 上下文：默认只发送选段所在段落；段落、小节和全文上下文均由服务端从会话绑定的不可变版本中派生，前端不能注入任意正文。全文上下文需要用户显式确认，并受字符上限约束。
- 决策：每个 Agent 会话同一时间只有一条待处理建议；成功生成下一版后，上一版标为 `superseded`。只有用户接受才创建新文稿版本，拒绝、继续调整和查看建议都不会改动正文或自动复检。
