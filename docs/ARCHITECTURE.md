...
- 文稿：500–5,000 个英文单词，稳定段落 ID，默认七天到期。
- 检测：单 Pangram 结果、模型/接口版本、三类概率、精确字符范围和明确免责声明；Mock 永远显示演示身份。
- 改写：V4-Pro 只返回结构化补丁，V4-Flash 独立检查语义漂移；同一 Agent 会话可以基于上一版建议继续调整，但原始选段始终是不变的安全锚点。规则保护数字、比例、部分引文/引用标记、URL 与大写缩写；不宣称所有公式、专名、事实均被硬性锁定，作者仍需审阅。
- 批量修改：当前有效检测的全部风险段落由服务端选择；真实 DeepSeek 和 Mock 都生成持久化的 pending 补丁预览，不直接写入正文。作者逐段勾选后，通过 `POST /rewrite-sessions/{id}/batch-decision` 一次接受所选修改，未选项保留原文；接受创建一个 `agent-first-pass` 不可变版本，检测立即过期。全部拒绝不创建版本。决策必须绑定当前基础版本，检查完整原段、保护内容、段落唯一性与所有权，并且不调用 Provider。旧生产行为需发布该分支后才改变。
- 上下文：默认只发送选段所在段落；段落、小节和全文上下文均由服务端从会话绑定的不可变版本中派生，前端不能注入任意正文。全文上下文需要用户显式确认，并受字符上限约束。
- 决策：普通对话会话同一时刻只有一条当前建议，下一版使前版 superseded；批量会话则每个段落有一条 pending 建议，统一选择后原子决策。两种流程都要求显式接受；查看、拒绝、接受和恢复不触发自动复检。批量预览的 Provider/Mock 身份随已有安全审计元数据保存，在文稿保留期内可刷新恢复。
- 版本：手工保存、补丁接受和恢复都创建新版本，不覆盖历史；导出先保存当前草稿，并携带 expected_version_id，版本冲突时拒绝导出旧稿。编辑中内容实时同步到内存草稿，退出/切换/恢复有保存、放弃、取消决策，不在浏览器持久存储论文。
- 文件：DOCX 经过文件名、MIME、ZIP、展开体积、条目、宏、路径和外部关系检查；按正文 XML 顺序提取段落与简单表格，表格转为文本行。公式、脚注/尾注、图片、修订、动态域、自动编号、页眉页脚正文和复杂表格会失败关闭；不保留原 Word 排版。
