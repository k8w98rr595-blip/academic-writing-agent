...
Detector trust boundary: Pangram responses are untrusted. Paperlight validates all three fractions and their sum, array size, version/task identifiers, labels, confidence values, character bounds, returned text, and every window substring before saving evidence. A failed Pangram call becomes a sanitized failure with no percentage or highlight; the provider response body is not exposed. Real detection stays disabled until `DETECTOR_DATA_PROCESSING_ACKNOWLEDGED=1` is deliberately set.
