...
Detector trust boundary: provider responses are untrusted. Paperlight validates scores, array sizes, model/task identifiers, character bounds, Pangram returned text, and Copyleaks scan identifiers before saving evidence. A failed Provider is represented as a sanitized failure object; its response body is not exposed, and the other Provider cannot become consensus evidence by itself. Real detection stays disabled until `DETECTOR_DATA_PROCESSING_ACKNOWLEDGED=1` is deliberately set.
