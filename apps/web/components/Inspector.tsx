...
function AgentPanel(props: Pick<Props, "selectedText" | "pendingPatch" | "instruction" | "busy" | "onInstruction" | "onPropose" | "onAccept" | "onReject">) {
  ...
  if (pendingPatch) {
    const providerLabel = pendingPatch.isMock
      ? "Mock Agent · review before accepting"
      : `${pendingPatch.provider || "Writing Agent"} ${pendingPatch.modelVersion || ""} · review before accepting`.replace(/\s+/g, " ");
    const validatorLabel = pendingPatch.validatorModelVersion
      ? `Semantically checked by ${pendingPatch.validatorModelVersion}`
      : pendingPatch.protectedStatus;
    return <div className="patch-panel"><div className="patch-title"><span className="patch-number">1</span><div><h2>Rewrite suggestion</h2><p>{providerLabel}</p></div></div><label>Original<blockquote>{pendingPatch.originalText}</blockquote></label><label>Suggested<blockquote className="suggested">{pendingPatch.revisedText}</blockquote></label><div className="patch-reason"><strong>Why this change?</strong><p>{pendingPatch.reason}</p></div><div className="protected-row"><ShieldCheck size={18} /><span>{validatorLabel}</span></div><div className="patch-actions"><button className="button danger-outline" disabled={busy} onClick={() => onReject(pendingPatch)}><X size={17} />Reject</button><button className="button primary" disabled={busy || pendingPatch.originalText === pendingPatch.revisedText} onClick={() => onAccept(pendingPatch)}><Check size={17} />Accept change</button></div></div>;
  }
